...
# Referência ao objeto do lápis que segue o mouse (ajuste se necessário)
...
#========== ENTRADA DE EVENTOS ==========
func _input(event):
	# Detecta cliques e movimentação do mouse para iniciar ou continuar o traço
...
#========== INICIAR NOVO TRAÇO ==========
func iniciar_linha():
	# Cria uma nova linha visual e prepara um corpo físico para futura colisão

	linha = Line2D.new()
	linha.default_color = Color.WHITE
	linha.width = 6.0
	linha.round_precision = 1.0
	...
	linha.add_point(inicio)  # Dupla inicial evita "piscada" na linha
...
func finalizar_linha():
	# Conclui o traço visual e gera a colisão física correspondente
	...
	if linha and pontos.size() >= 2:
		...
		for p in pontos:
			pontos_locais.append(p - origem)  # Converte pontos globais em locais

		var colisao := CollisionPolygon2D.new()
		colisao.polygon = gerar_poligono(pontos_locais, linha.width)

		corpo.position = origem
		corpo.add_child(colisao)

		elementos_desenhados.append_array([linha, corpo])
	else:
...
func gerar_poligono(pontos_base: Array, largura: float) -> PackedVector2Array:
	# Gera um contorno ao redor da linha com base nos pontos e largura
	...
	for i in pontos_base.size():
		...
		if i < pontos_base.size() - 1:
			var direcao: Vector2 = (pontos_base[i + 1] - pontos_base[i]).normalized()
			normal = direcao.orthogonal()

		esquerda.append(pontos_base[i] + normal * metade)
		direita.insert(0, pontos_base[i] - normal * metade)

	return PackedVector2Array(esquerda + direita)
#====================================
